...
class WMXICore {
	...
	public function __construct($cainfo = '', $encoding = 'UTF-8') {
		if (!empty($cainfo) && !file_exists($cainfo)) { throw new WMXIException("Specified certificates dir $cainfo not found."); }
		$this->cainfo = $cainfo;
	...
	public function Classic($wmid, $key) {
		...
		$this->light   = false;
		$this->wmid    = $wmid;
		if (!class_exists('WMSigner')) { throw new WMXIException('WMSigner class not found.'); }
	...
	protected function _sign($text) {
		if (!$this->classic) { throw new WMXIException('Classic initialization required to sign the string.'); }
